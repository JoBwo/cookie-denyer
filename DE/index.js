...
function setProgress(){
	progress.innerHTML = "Abgelehnte Cookies " + currentCookie + "/10";
}
...
function nextCookie(){
	if(currentCookie == 9){
		...
		win.innerHTML = "<h1>Du hast GEWONNEN!</h1>Du hast dich " + penalties + " Mal verklickt.<br>Deine Zeit lautet " + (timer/100.0) + " Sekunden.<br><br><button onclick='restart()'>VON VORNE</button>"
		return;
	}
	cookieWindows[currentCookie].classList.add("hide");
	currentCookie++;
...
function gameOver(){
	...
	gameover.innerHTML = "Du hast verkackt.<br>Ich hab dir eine ganz leichte Aufgabe gegeben und du schaffst nicht mal Level 1.";
	progressbar.classList.toggle("hide");
	wrapper.style.top = "0px";
	wrapper.style.height = "100%";
...
function addPenalty(){
	penalties++;
	penalty.innerHTML = "Strafen: " + penalties;
...
function c3makeShureChecked(){
	...
	document.querySelector(".c3-input").value="Ja";
}
...
function c3checkOptions(){
	...
	if(document.querySelector(".c3-input").value != "NEIN!1!"){
		unchecked = false;
	}
